...
def tokenize_paragraph(paragraph):
    TOKENS = [
        ...
        ('PLUS', r'(?P<PLUS>\+)'),
        ('EQUALS', r'(?P<EQUALS>=)'),
        ('POSS_LINE_REF', r'(?P<POSS_LINE_REF> \d{1,2}[;. ])')  # catches digits 1-99 that may be isolated line numbers
    ]
    ...
    for i, (name, content, span) in enumerate(tokens):
        if name == 'FULL_XREF':
            ...
            if previous_xref == None:
                if last_xref_before_Klammer == None:
                    logging.warning(f"Discarded xref {content} without prev. xref in: {paragraph[span[0]-20:span[1]+15]}")
                    tagged_tokens.append(('TEXT', content, span))
                    continue
                # else:  # we are in a parenthesis, so supply the last xref from last_full_xref
...
# as long as no invalid sequences intervene. Otherwise return None.
